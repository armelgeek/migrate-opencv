...
class OpenCVCanvas:
    ...
    def draw_line(self, start: Tuple[int, int], end: Tuple[int, int], 
        ...
        """
        # OpenCV uses BGR, convert RGBA → BGR
        bgr_color = (color[2], color[1], color[0])
        
        # Handle alpha blending
        if color[3] < 255:
            overlay = self.image.copy()
            cv2.line(overlay, start, end, bgr_color + (255,), thickness, cv2.LINE_AA)
            alpha = color[3] / 255.0
            cv2.addWeighted(overlay, alpha, self.image, 1 - alpha, 0, self.image)
        else:
            cv2.line(self.image, start, end, bgr_color + (255,), thickness, cv2.LINE_AA)
    ...
    def draw_polylines(self, points: np.ndarray, color: Tuple[int, int, int, int], 
        ...
        if color[3] < 255:
            overlay = self.image.copy()
            cv2.polylines(overlay, [points], closed, bgr_color + (255,), thickness, cv2.LINE_AA)
            alpha = color[3] / 255.0
            cv2.addWeighted(overlay, alpha, self.image, 1 - alpha, 0, self.image)
        else:
            cv2.polylines(self.image, [points], closed, bgr_color + (255,), thickness, cv2.LINE_AA)
